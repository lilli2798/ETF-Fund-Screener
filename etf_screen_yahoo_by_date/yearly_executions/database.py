...
class YearlyReturnsDatabase:
    ...
    def save_analysis_results(self, result_df: pd.DataFrame):
        ...
        for ticker in result_df.index:
            # Save main analysis results (skip tickers with no data)
            if pd.isna(result_df.loc[ticker, 'no_null_columns']) or result_df.loc[ticker, 'no_null_columns'] == 0:
                continue
            ...
            """, (
                ticker,
                int(result_df.loc[ticker, 'total_columns']) if pd.notna(result_df.loc[ticker, 'total_columns']) else 0,
                int(result_df.loc[ticker, 'no_null_columns']) if pd.notna(result_df.loc[ticker, 'no_null_columns']) else 0,
                int(result_df.loc[ticker, 'better']) if pd.notna(result_df.loc[ticker, 'better']) else 0,
                int(result_df.loc[ticker, 'worst']) if pd.notna(result_df.loc[ticker, 'worst']) else 0,
                float(result_df.loc[ticker, 'better_%']) if pd.notna(result_df.loc[ticker, 'better_%']) else 0.0,
                float(result_df.loc[ticker, 'worst_%']) if pd.notna(result_df.loc[ticker, 'worst_%']) else 0.0,
                int(result_df.loc[ticker, 'better_worst_diff']) if pd.notna(result_df.loc[ticker, 'better_worst_diff']) else 0
            ))
            ...
            percentile_values = [int(result_df.loc[ticker, col]) if pd.notna(result_df.loc[ticker, col]) else 0 for col in percentile_cols]
            cursor.execute("""
