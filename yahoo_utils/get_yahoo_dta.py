        ...
        try:
            ...
            if holdings_df is not None and not holdings_df.empty:
                weight_matrix = {
                    ...
                    "Technology - Semiconductors (Chips)": 0.0,
                    "Defensive - Healthcare": 0.0,
                    "Financials / Banking": 0.0,
                    "Broad Market Equity": 0.0
                }
                ...
                for stock_symbol in top_stocks[:loop_limit]:
                    try:
                        s_info = yf.Ticker(str(stock_symbol)).info
                        if not s_info:
                            continue
                        ...
                        if "software" in ind_key or "internet-content" in ind_key:
                            weight_matrix["Technology - Software & Services"] += holding_weight
                        elif "semiconductor" in ind_key:
                            weight_matrix["Technology - Semiconductors (Chips)"] += holding_weight
                        elif "healthcare" in sec_key or "medical" in ind_key:
                            weight_matrix["Defensive - Healthcare"] += holding_weight
                        elif "financial" in sec_key or "insurance" in ind_key:
                            weight_matrix["Financials / Banking"] += holding_weight
                        else:
                            weight_matrix["Broad Market Equity"] += holding_weight
                    except Exception:
                        continue
